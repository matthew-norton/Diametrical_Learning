...
from datastore import get_MNIST
import sys
...
input_shape = (28,28) #need shape of the input data for later (when making network, need this for size of first layer in fc_net.Simple_Net)
train_loader, test_loader = get_MNIST(path,
                                        exclude_list,
                                        batch_size_train,
                                        batch_size_test,
                                        percent_flip)
...
sampling_interval = 5
num_epochs =351
lr_schedule = dict([ (0,.01) , (300,.001)] )
gamma_schedule = dict( [(0,10) ] )
